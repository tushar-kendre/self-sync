...
export default function SleepLogScreen() {
  ...
  // Form state
  const [bedTime, setBedTime] = React.useState<Date>(new Date());
  const [wakeTime, setWakeTime] = React.useState<Date>(new Date());
  const [showBedTimePicker, setShowBedTimePicker] = React.useState(false);
  const [showWakeTimePicker, setShowWakeTimePicker] = React.useState(false);
  ...
  const [todaySleepLog, setTodaySleepLog] = React.useState<SleepLog | null>(
    null,
  );
  ...
  const getGradientColors = (
    ...
    const gradients = {
      ...
      formHeader: isDark
        ? (["#581c87", "#312e81"] as const)
        : (["#8b5cf6", "#6366f1"] as const),
      motivational: isDark
        ? (["#581c87", "#312e81"] as const)
        : (["#f3e8ff", "#e0e7ff"] as const),
      summary: isDark
        ? (["#1e40af", "#581c87"] as const)
        : (["#dbeafe", "#f3e8ff"] as const),
      encouragement: isDark
        ? (["#be185d", "#ec4899"] as const)
        : (["#fdf2f8", "#fce7f3"] as const),
      sleepLogged: isDark
        ? (["#166534", "#0f766e"] as const)
        : (["#f0fdf4", "#f0fdfa"] as const),
      sleepTips: isDark
        ? (["#0f766e", "#0891b2"] as const)
        : (["#f0fdfa", "#ecfeff"] as const),
      quickAdd: isDark
        ? (["#312e81", "#581c87"] as const)
        : (["#6366f1", "#8b5cf6"] as const),
      bottomEncouragement: isDark
        ? (["#581c87", "#7c3aed"] as const)
  ...
  const calculateSleepDuration = (bedTime: Date, wakeTime: Date): number => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Format times as HH:MM strings for database
      const bedTimeString = bedTime.toTimeString().slice(0, 5);
      const wakeTimeString = wakeTime.toTimeString().slice(0, 5);
  ...
  const getMotivationalMessage = () => {
    ...
    if (hour < 6) {
      return "Still up? Time for rest! 🌙";
    } else if (hour < 12) {
      return "Good morning! How did you sleep? ☀️";
    } else if (hour < 18) {
      return "Good afternoon! Planning your rest? 🌤️";
    } else {
      return "Good evening! Ready to track sleep? 🌅";
    }
  };
  ...
  const renderSleepSummary = () => {
    ...
    const quality = sleepQualityOptions.find(
      (q) => q.value === todaySleepLog.sleepQuality,
    );
          ...
          <CardContent className="pt-4">
            ...
            <View className="gap-2">
              ...
              <View className="flex-row justify-between">
                ...
                <Text
                  className={`text-sm font-medium ${getSleepDurationColor(duration)}`}
                >
        ...
        <ScrollView className="flex-1">
          ...
          <View className="gap-6 px-6 py-6 -mt-4">
            ...
            <TouchableOpacity
              ...
              style={{
                opacity: todaySleepLog !== null ? 0.5 : 1,
                shadowColor: "#8b5cf6",
                shadowOffset: { width: 0, height: 4 },
                shadowOpacity: todaySleepLog !== null ? 0 : 0.3,
                shadowRadius: 8,
              }}
            >
              <LinearGradient
                ...
                >
                  <CardContent className="pt-4">
                    ...
                    <Text className="text-green-700 dark:text-green-300 text-sm">
                      Fantastic! You've already tracked your sleep today. Keep
                      building those healthy habits! Tomorrow's another
                      opportunity for great rest! 🌟
                    </Text>
                  </CardContent>
                </LinearGradient>
              </Card>
      ...
      <ScrollView className="flex-1">
        ...
        >
          <View className="px-6 pb-6">
            ...
            <Text className="text-white/90 dark:text-white/80 text-lg mb-2">
              Track your sleep for better rest! 🌟
            </Text>
            <Text className="text-white/70 dark:text-white/60 text-sm">
              {getCurrentTime()} • {new Date().toLocaleDateString()}
            </Text>
        ...
        <View className="gap-6 px-6 py-6 -mt-4">
          {/* Encouragement Card */}
          <Card className="w-full shadow-sm border-pink-200 dark:border-pink-800 rounded-lg overflow-hidden">
            <LinearGradient
            ...
            >
              <CardContent className="pt-4">
                ...
                <Text className="text-pink-700 dark:text-pink-300 text-sm mt-2">
                  Sleep tracking shows you care about wellness! 💤✨
                </Text>
              </CardContent>
            </LinearGradient>
          </Card>
            ...
            <CardHeader className="pb-4">
              <CardTitle className="flex-row items-center gap-2">
                ...
                <View className="flex-1">
                  <Text className="text-lg font-semibold text-indigo-800 dark:text-indigo-200 leading-6">
                    Sleep Times 🕘
                  </Text>
                </View>
              </CardTitle>
            ...
            <CardContent className="pt-0 gap-4">
              <View>
                ...
                <TouchableOpacity
                  ...
                  className="border-2 border-indigo-200 dark:border-indigo-700 rounded-xl p-4 bg-white dark:bg-gray-900/50 flex-row items-center justify-between"
                >
              ...
              <View className="bg-indigo-50 dark:bg-indigo-950/30 p-4 rounded-xl">
                <Text className="text-center text-indigo-800 dark:text-indigo-200 font-medium">
                  Sleep Duration:{" "}
                  {calculateSleepDuration(bedTime, wakeTime).toFixed(1)} hours
                </Text>
              </View>
            </CardContent>
          </Card>
          ...
          <Card className="w-full shadow-sm border border-blue-200 dark:border-blue-800">
            <CardHeader className="pb-4">
              ...
              <Text className="text-sm text-muted-foreground">
                Any thoughts about your sleep? Everything you notice matters!
                (Optional)
              </Text>
            </CardHeader>
            <CardContent className="pt-0">
          ...
          <View className="gap-4">
            <TouchableOpacity
              onPress={handleSave}
              disabled={sleepQuality === null || isLoading}
              className="w-full shadow-lg rounded-xl overflow-hidden"
              style={{
                opacity: sleepQuality === null || isLoading ? 0.5 : 1,
                shadowColor: "#8b5cf6",
                shadowOffset: { width: 0, height: 4 },
                shadowOpacity: sleepQuality === null || isLoading ? 0 : 0.3,
                shadowRadius: 8,
              }}
            >
              <LinearGradient
